...
public class JobHuntPostDAO { // 구인 게시판 DAO
...
// ----------------------------------------------------------------- 메인 페이지 SELECTALL -----------------------------------------------------------------
 ...
 // 프리미엄 회원이 작성한 구직글 출력 (최신글 2개만 출력).노승현
 ...
 // 사용한 컬럼 (출력 내용) : 카테고리, 구직글 아이디, 회원 아이디, 회원 닉네임(회원 테이블), 제목, 작성일, 대표 이미지(게시글 이미지 테이블)
 // 쿼리문 설명 :
 ...
 private static final String SELECTALL_JOBHUNTPOSTWEEKLYBEST = "SELECT "
		  ...
		  + "	RECOMMEND ON JOBHUNTPOST.JOBHUNTPOST_id = RECOMMEND.POST_id " // 좋아요 정보와 LEFT JOIN
		  + "WHERE "
		  + "	JOBHUNTPOST.JOBHUNTPOST_date >= DATE_SUB(NOW(), INTERVAL 1 WEEK) "
		  + "GROUP BY "
		  + "	JOBHUNTPOST.JOBHUNTPOST_id "
		  + "ORDER BY "
		  + "	COUNT(RECOMMEND.POST_id) DESC, " // (1) 좋아요 수가 많은 순으로 정렬한 뒤
		  + "	JOBHUNTPOST.JOBHUNTPOST_date DESC " // (2) 다음 작성일이 최신인 순으로 정렬
		  + "LIMIT 2 ";
 // 사용한 테이블 : 구직글 테이블, 회원 테이블, 좋아요 테이블, 게시글 이미지 테이블
 // 사용한 컬럼 (출력 내용) : 카테고리, 구직글 아이디, 회원 아이디, 회원 닉네임(회원 테이블), 제목, 내용, 작성일, 좋아요 수(좋아요 테이블), 대표 이미지(게시글 이미지 테이블)
 // 쿼리문 설명 :
 // INNER JOIN을 사용해 구직글 테이블과 회원 테이블을 연결하고, 또 다른 LEFT JOIN을 사용해 구직글 테이블과 좋아요 테이블을 연결
...
// ----------------------------------------------------------------- 구직글 페이지 SELECTALL -----------------------------------------------------------------
 ...
 // 프리미엄 회원이 작성한지 한 달 이내의 글 목록 출력.노승현
 private static final String SELECTALL_JOBHUNTPOSTPREMIUM1MONTH = "SELECT "
		  + "DISTINCT " // 중복 제거 함수
		  + "	'JobHuntPost' AS POST_category, " // 게시판 카테고리 설정
		  + "	JOBHUNTPOST.JOBHUNTPOST_id, "
		  + "	JOBHUNTPOST.MEMBER_id, "
		  ...
		  + "	AND JOBHUNTPOST.JOBHUNTPOST_date >= DATE_SUB(NOW(), INTERVAL 1 MONTH) " // 작성일이 한 달 이내인 경우
		  + "ORDER BY "
		  + "   JOBHUNTPOST.JOBHUNTPOST_date DESC "; // 작성일을 기준으로 내림차순 정렬
 // 사용한 테이블 : 구직글 테이블, 회원 테이블, 좋아요 테이블, 게시글 이미지 테이블
 ...
 // 게시글 이미지는 테이블을 따로 나누었으며 서브 쿼리를 사용해 게시글 이미지 중 대표 이미지로 보여줄 이미지를 설정하고, 그 결과를 "POSTIMG_name"라는 별칭으로 반환
  ...
  // 사용한 테이블 : 구직글 테이블, 회원 테이블, 좋아요 테이블, 게시글 이미지 테이블
  // 사용한 컬럼 (출력 내용) : 카테고리, 구직글 아이디, 회원 아아디, 회원 닉네임(회원 테이블), 제목, 내용, 작성일, 조회수, 좋아요 수(좋아요 테이블), 대표 이미지(게시글 이미지 테이블)
  // 쿼리문 설명 :
  // INNER JOIN을 사용해 구직글 테이블과 회원 테이블을 연결하고, 또 다른 LEFT JOIN을 사용해 구직글 테이블과 좋아요 테이블을 연결
  // 게시글의 좋아요는 테이블을 따로 나누었으며 COUNT() 함수를 사용해 게시글에 대한 좋아요 수를 합산하고, 그 결과를 "RECOMMEND_cnt"라는 별칭으로 반환
  // 게시글 이미지는 테이블을 따로 나누었으며 서브 쿼리를 사용해 게시글 이미지 중 대표 이미지로 보여줄 이미지를 설정하고, 그 결과를 "POSTIMG_name"라는 별칭으로 반환
...
// ----------------------------------------------------------------- 회원 페이지 SELECTALL -----------------------------------------------------------------
  ...
  private static final String SELECTALL_JOBHUNTPOSTMEMBER = "SELECT "
		  ...
		  + "	JOBHUNTPOST.JOBHUNTPOST_id DESC "; // 게시글 아이디를 기준으로 내림차순 정렬
  // 사용한 테이블 : 구직글 테이블, 회원 테이블, 좋아요 테이블, 게시글 이미지 테이블
  // 사용한 컬럼 (출력 내용) : 카테고리, 구직글 아이디, 회원 닉네임(회원 테이블), 제목, 내용, 작성일, 조회수, 좋아요 수(좋아요 테이블), 대표 이미지(게시글 이미지 테이블)
  // 쿼리문 설명 :
  // INNER JOIN을 사용해 구직글 테이블과 회원 테이블을 연결하고, 또 다른 LEFT JOIN을 사용해 구직글 테이블과 좋아요 테이블을 연결
  // 게시글의 좋아요는 테이블을 따로 나누었으며 COUNT() 함수를 사용해 게시글에 대한 좋아요 수를 합산하고, 그 결과를 "RECOMMEND_cnt"라는 별칭으로 반환
  // 게시글 이미지는 테이블을 따로 나누었으며 서브 쿼리를 사용해 게시글 이미지 중 대표 이미지로 보여줄 이미지를 설정하고, 그 결과를 "POSTIMG_name"라는 별칭으로 반환
...
// --------------------------------------------------------------- INSERT, UPDATE, DELETE ---------------------------------------------------------------
  ...
  public List<JobHuntPostDTO> selectAll(JobHuntPostDTO jobHuntPostDTO) {
	  ...
	  try {
		  ...
		  else if (jobHuntPostDTO.getSearchCondition().equals("jobHuntPostPremium1MonthList")) {
			  result = jdbcTemplate.query(SELECTALL_JOBHUNTPOSTPREMIUM1MONTH, new JobHuntPostPremium1MonthRowMapper());
			  System.out.println("JobHuntPostDAO(selectAll) Out로그 = [" + result + "]");
			  return result;
		  }		  
		  // 구직글 페이지 - 구직글 전체 출력
		  else if (jobHuntPostDTO.getSearchCondition().equals("jobHuntPostList")) {
			  result = jdbcTemplate.query(SELECTALL_JOBHUNTPOST, new JobHuntPostRowMapper());
			  System.out.println("JobHuntPostDAO(selectAll) Out로그 = [" + result + "]");
			  return result;
		  }
...
// 메인 페이지 - 프리미엄 회원이 작성한 구직글 목록 출력 시 필요한 데이터를 저장할 RowMapper 클래스.노승현
...
// 메인 페이지 - 주간 추천순 구직글 목록 출력 시 필요한 데이터를 저장할 RowMapper 클래스.노승현
...
// 구직글 페이지 - 프리미엄 회원이 작성한지 한 달 이내의 글 필요한 데이터를 저장할 RowMapper 클래스.노승현
class JobHuntPostPremium1MonthRowMapper implements RowMapper<JobHuntPostDTO> {
	@Override // mapRow 메서드 오버라이드
	public JobHuntPostDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
		// ResultSet에 저장된 데이터를 JobHuntPostDTO 객체에 매핑(저장)하는 메서드
...
// 구직글 페이지 - 구직글 목록 출력 시 필요한 데이터를 저장할 RowMapper 클래스.노승현
...
// 회원 페이지 - 특정 회원이 작성한 구직글 목록 출력 RowMapper 클래스.노승현
...
// 게시글 이미지 저장 시 필요한 게시글 아이디의 최대값을 저장할 RowMapper 클래스.노승현
...
// 메인 페이지 - 최신 게시글 목록 출력 RowMapper 클래스.노승현
...
// 구직글 페이지 - 구직글 상세 출력 시 필요한 데이터를 저장할 RowMapper 클래스.노승현
