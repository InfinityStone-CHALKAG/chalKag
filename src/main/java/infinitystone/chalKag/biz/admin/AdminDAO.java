...
public class AdminDAO {
  ...
  private static final String SELECTALL_SIGNINCOUNTBYYEARMONTHDATE = "WITH RECURSIVE DATERANGE AS ( " +
      ...
      "FROM DATERANGE " +
      "WHERE DATE < LAST_DATE(DATE) " +
      ")" +
      "SELECT" +
      "DAY(D.DATE) AS DATEOFMONTH, " +
      "COALESCE(COUNT(M.MEMBER_id), 0) AS SIGNINCOUNT " +
      "FROM " +
      "DATERANGR D " +
      "LEFT JOIN SIGNINLOG ON DATE(SIGNINLOG.SIGNINLOG_date) = D.DATE " +
      "AND YEAR(SIGNINLOG.SIGNINLOG_date) = ? " +
      "AND LPAD(MONTH(S.SIGNINLOG_date), 2, '0') = LPAD(?, 2, '0') " +
