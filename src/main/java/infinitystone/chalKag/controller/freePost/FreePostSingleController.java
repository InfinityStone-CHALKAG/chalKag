...
public class FreePostSingleController {
		...
		// "/freePostSingle" 요청이 들어오면 해당 요청을 처리하기 위해 freePostList 메서드가 호출되며,
		// 호출된 메서드로 서치 조건에 따라 글 목록을 조회한 후 JSON 형식으로 변환하여 모델에 저장
		// 저장한 데이터는 "freePost/freePostSingle" 뷰로 반환	
	...
	public String freePostList(HttpSession session, Model model, HeadHuntPostDTO headHuntPostDTO, CommentDTO commentDTO, PostImgDTO postImgDTO, RecommendDTO recommendDTO, MarketPostDTO marketPostDTO,JobHuntPostDTO jobHuntPostDTO, FreePostDTO freePostDTO) {
		...
		recommendDTO.setPostId(freePostDTO.getFreePostId());
		
		recommendDTO.setMemberId((String)session.getAttribute("member"));
	
		recommendDTO = recommendService.selectOne(recommendDTO);
		
		FreePostDTO latedto = new FreePostDTO();
		latedto.setSearchCondition("freePostRecentPostSingle");
		
		FreePostDTO latestFreePost  = freePostService.selectOne(latedto);
