	...
	function displayReviewData(pageDatas) {
		...
		if (pageDatas.length <= 0) {
			...
			pageDatas.forEach(function(marketPostList) {
				...
				console.log("marketPostList.myRecommend : " + marketPostList.myRecommend);
				var isAlreadyRecommended = marketPostList.myRecommend === 1; // 1이면 true, 그 외에는 false
				...
				if (!memberId) {
					recommendButtonHTML = `<a id="recommendBtnMessage" onclick="message()" class="love" style="margin-top:0%">
                            ...
                            <div id="recommendCnt">${marketPostList.recommendCnt}</div>
                         </a>`;
				} else  {
					recommendButtonHTML = `<a id="recommendBtn_${marketPostList.marketPostId}" class="love ${isAlreadyRecommended ? 'active' : ''}" style="margin-top:0%" data-postid="${marketPostList.marketPostId}" data-memberid="${memberId}">
