	...
	function displayReviewData(pageDatas) {
		...
		if (pageDatas.length <= 0) {
			...
			pageDatas.forEach(function(jobHuntPostList) {
				...
				console.log("jobHuntPostList.myRecommend : " + jobHuntPostList.myRecommend);
				var isAlreadyRecommended = jobHuntPostList.myRecommend === 1; // 1이면 true, 그 외에는 false
				...
				if (!memberId) {
					recommendButtonHTML = `<a id="recommendBtnMessage" onclick="message()" class="love" style="margin-top:0%">
                            ...
                            <div id="recommendCnt">${jobHuntPostList.recommendCnt}</div>
                         </a>`;
				} else  {
					recommendButtonHTML = `<a id="recommendBtn_${jobHuntPostList.jobHuntPostId}" class="love ${isAlreadyRecommended ? 'active' : ''}" style="margin-top:0%" data-postid="${jobHuntPostList.jobHuntPostId}" data-memberid="${memberId}">
               		...
               		<div class="inner">
                        <figure>
                        	<a href="/jobHuntPostSingle?jobHuntPostId=${jobHuntPostList.jobHuntPostId}">
                               <img src="/postImg/${jobHuntPostList.postImgName}"  style="width: 100%; height: 100%; object-fit: cover;">
                            </a>
                        </figure>
                        <div class="details">
                            <div class="detail">
                            ...
                            <h1 style="width: 522.5px; max-height: 56px; overflow: hidden; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2;"><a href="/jobHuntPostSingle?jobHuntPostId=${jobHuntPostList.jobHuntPostId}">${jobHuntPostList.jobHuntPostTitle}</a></h1>
                             <p style="width: 522.5px; max-height: 52px; overflow: hidden; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2;">${jobHuntPostList.jobHuntPostContent}</p>
                            <footer>
                                ${recommendButtonHTML}
